...
class ParseTableCRUD
{
    ...
    public function __construct(public $database, public string $tableName, public array $fields, public string $tableComment)
    {
        ...
        $this->pathPrefix = Func::getPrefixBeforeUnderscore($this->tableName);
    ...
    private function createLists(): void
    {
        ...
        $this->saveStr[] = '    {';
        $this->saveStr[] = '        $page = $request->getExt()?->getPage() ?: 1;';
        $this->saveStr[] = '        $size = $request->getExt()?->getSize() ?: 10;';
        $this->saveStr[] = '';
        $this->saveStr[] = '        $where = [];';
        $this->saveStr[] = '        $order = [' . $this->tableName . 'Table::PRI_KEY=>"desc"];';
        $this->saveStr[] = '        $' . lcfirst($this->tableName) . 'Table = new ' . $this->tableName . 'Table();';
        $this->saveStr[] = '        $dto = $' . lcfirst($this->tableName) . 'Table->order($order)->where($where)->page($page, $size)->selectAll();';
        $this->saveStr[] = '';
