...
class ValidationAspect extends AbstractAspect implements ProxyAttributeInterface
{
        ...
        foreach ($this->rules as $index => $rules) {
            if (!isset($arguments[$index])) {
                if (in_array('required', $rules)) {
                    throw new AppException("参数 #$index " . LanguageEnum::PARAM_REQUIRED);
                }
                continue;
            }
    ...
    {
        if (!is_string($value)) {
            throw new AppException("参数 #$index " . LanguageEnum::PARAM_MUST_STRING);
        }
    }
    ...
    private function validateInt(mixed $value, int $index): void
    {
        if (!is_int($value) && !ctype_digit((string)$value)) {
            throw new AppException("参数 #$index " . LanguageEnum::PARAM_MUST_INT);
        }
    }
    ...
    {
        if (!is_numeric($value)) {
            throw new AppException("参数 #$index " . LanguageEnum::PARAM_MUST_NUMBER);
        }
    }
    ...
    private function validateEmail(mixed $value, int $index): void
    {
        if (!filter_var($value, FILTER_VALIDATE_EMAIL)) {
            throw new AppException("参数 #$index " . LanguageEnum::PARAM_MUST_EMAIL);
        }
    }
    ...
    private function validateUrl(mixed $value, int $index): void
    {
        if (!filter_var($value, FILTER_VALIDATE_URL)) {
            throw new AppException("参数 #$index " . LanguageEnum::PARAM_MUST_URL);
        }
    }
    ...
    private function validateMin(mixed $value, ?string $min, int $index): void
    {
        ...
        if (is_string($value)) {
            if (mb_strlen($value) < $minValue) {
                throw new AppException("参数 #$index " . LanguageEnum::PARAM_MIN_LENGTH . " $minValue");
            }
        } elseif (is_numeric($value)) {
            if ($value < $minValue) {
                throw new AppException("参数 #$index " . LanguageEnum::PARAM_MIN_VALUE . " $minValue");
            }
        } elseif (is_array($value)) {
            if (count($value) < $minValue) {
                throw new AppException("参数 #$index " . LanguageEnum::PARAM_MIN_COUNT . " $minValue");
            }
        }
    }
    ...
    private function validateMax(mixed $value, ?string $max, int $index): void
    {
        ...
        if (is_string($value)) {
            if (mb_strlen($value) > $maxValue) {
                throw new AppException("参数 #$index " . LanguageEnum::PARAM_MAX_LENGTH . " $maxValue");
            }
        } elseif (is_numeric($value)) {
            if ($value > $maxValue) {
                throw new AppException("参数 #$index " . LanguageEnum::PARAM_MAX_VALUE . " $maxValue");
            }
        } elseif (is_array($value)) {
            if (count($value) > $maxValue) {
                throw new AppException("参数 #$index " . LanguageEnum::PARAM_MAX_COUNT . " $maxValue");
            }
        }
    }
    ...
    {
        if (!is_array($value)) {
            throw new AppException("参数 #$index " . LanguageEnum::PARAM_MUST_ARRAY);
        }
    }
    ...
    private function validateIn(mixed $value, ?string $values, int $index): void
    {
        ...
        if (!in_array($value, $allowedValues, true)) {
            throw new AppException("参数 #$index " . LanguageEnum::VALUE_INVALID . ": 必须是以下值之一: " . implode(', ', $allowedValues));
        }
    }
    ...
    private function validateRegex(mixed $value, ?string $pattern, int $index): void
    {
        ...
        if (!preg_match($pattern, (string)$value)) {
            throw new AppException("参数 #$index " . LanguageEnum::FORMAT_INVALID);
        }
    }
