...
class MessageQueue
{
    ...
    public static function pushProxy(array $data): int
    {
        ...
        $id = $reflection->newInstance()->insert([
            $reflection->getConstant("NEXT_RUN_TIME") => time() + $delay,
            $reflection->getConstant("MAX_NUM") => $maxRetry,
            $reflection->getConstant("DELAY_TIMES") => json_encode($retryIntervals, JSON_UNESCAPED_UNICODE),
    ...
    private static function exec($messageQueueTableDto): void
    {
        ...
        try {
            if ($messageQueueTableDto->runNum >= $messageQueueTableDto->maxNum) {
                $reflection->newInstance()->addWhere($reflection->getConstant("ID"), $messageQueueTableDto->id)->update([
                    $reflection->getConstant("IS_DISCARD") => 1,
            ...
            try {
                ...
                $reflection->newInstance()->addWhere($reflection->getConstant("ID"), $messageQueueTableDto->id)->update([
                    $reflection->getConstant("RUN_NUM") => Db::incr(),
    ...
    {
        $reflection = Db::getTableReflection('MessageQueueTable');
        $affectedRows = $reflection->newInstance()
            ->addWhere($reflection->getConstant("ID"), $msgId)
            ->addWhere($reflection->getConstant("IS_DISCARD"), 0) // 只取消未被丢弃的队列
            ->addWhere($reflection->getConstant("IS_SUCCESS"), 0) // 只取消未成功的队列
            ->update([
                $reflection->getConstant("IS_DISCARD") => 1,
                $reflection->getConstant("ERROR") => '手动取消',
                $reflection->getConstant("LAST_RUN_TIME") => time(),
            ]);
