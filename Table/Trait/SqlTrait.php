...
trait SqlTrait
{
     ...
     * @throws Throwable 当查询执行失败时抛出异常
     */
    public function min(string $field): string|int
    {
        $this->queryBuild->min($field);
        $res = $this->find();
        return $res ? ($res['num'] ?: 0) : 0;
    }
    ...
    {
        $this->queryBuild->count($field, $distinct);
        $res = $this->find();
        return $res && isset($res['num']) ? (int)$res['num'] : 0;
    }
