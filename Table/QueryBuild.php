...
class QueryBuild
{
     ...
     * 设置查询结果的排序方式
     *
     * @param array $order 排序配置数组，键为字段名，值为排序方式(ASC/DESC)或Expression对象
     *                     支持以下格式：
     ...
     * @return void 返回当前实例以支持链式调用
     */
    public function group(string $field = ''): void
    {
        if ($field) {
            $field = $this->table->formatField($field);
     ...
     */
    public function max(string $field): void
    {
        $field = $this->table->formatField($field);
     ...
     * @throws Throwable 当查询执行失败时抛出异常
     */
    public function min(string $field): void
    {
        $field = $this->table->formatField($field);
     ...
     * @throws Throwable 当查询执行失败时抛出异常
     */
    public function sum(string $field): void
    {
        // 匹配字段名的正则表达式
        ...
        $this->_field = "SUM($processedField) as num";
        $this->find();
     ...
     * @throws Throwable 当查询执行失败时抛出异常
     */
    public function avg(string $field): void
    {
        // 匹配字段名的正则表达式
        $pattern = '/\b([a-zA-Z_][a-zA-Z0-9_]*)\b/';
        ...
        $this->_field = "AVG($processedField) as num";
        $this->find();
     ...
     * @throws Throwable 当查询执行失败时抛出异常
     */
    public function count(string $field = '*', bool $distinct = false): void
    {
        $field = $this->table->formatField($field);
        if ($distinct) {
